...
namespace Mechanical.MagicBag
{
    ...
    public static class MagicBag
    {
        ...
        internal static void CreateDefault( IMagicBag parentBag )
        {
            ...
            var mappings = new Mapping[0];

            if( parentBag.NullReference() )
                Interlocked.CompareExchange(ref defaultBag, new Basic(mappings, MappingGenerators.Defaults), comparand: null);
            else
                Interlocked.CompareExchange(ref defaultBag, new Supplement(parentBag, mappings, MappingGenerators.Defaults), comparand: null);
        }
