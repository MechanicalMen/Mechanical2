...
{
    /// <summary>
    /// Generates <see cref="Func{T}"/> mappings.
    /// </summary>
    public class FuncGenerator : IMappingGenerator
    {
        #region Allocator

        private class Allocator<T> : IMagicBagAllocator<Func<T>>
        {
            ...
            public Func<T> CreateInstance( IMagicBag magicBag )
            {
                return new Func<T>(() => (T)this.mapping.Get(magicBag));
            }
        }
        ...
        public Mapping[] Generate( Mapping[] mappings )
        {
            ...
            for( int i = 0; i < array.Length; ++i )
            {
                ...
                var generatedType = typeof(Func<>).MakeGenericType(originalMapping.From);
