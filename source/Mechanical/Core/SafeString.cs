    ...
    public static class SafeString
    {
        ...
        public static class FormatProvider
        {
            ...
            public class Debug : Enumerable
            {
                ...
                private void AppendException( StringBuilder sb, string format, Exception e )
                {
                    sb.Append("Type: ");
                    this.AppendType(sb, format, e.GetType());

                    if( !e.Message.NullOrWhiteSpace() )
                    {
                        sb.AppendLine();
                        sb.Append("Message: ");
                        sb.Append(e.Message);
                    ...
                    if( enumerator.MoveNext() )
                    {
                        ...
                        do
                        {
                            ...
                            sb.Append(' ', 2);
                            sb.Append(SafeString.Print(enumerator.Current.Key));
                            sb.Append(" = ");
                            sb.Append(SafeString.Print(enumerator.Current.Value));
                            //// no newline here
                        }
                        while( enumerator.MoveNext() );
                    }

                    if( !e.StackTrace.NullOrWhiteSpace() ) // this can actually happen
                    {
                        sb.AppendLine();
                        sb.AppendLine();
                    ...
                    if( e.InnerException.NotNullReference() )
                    {
                        var aggregateException = e as AggregateException;
                        if( aggregateException.NullReference() )
                        {
                            sb.AppendLine();
                            sb.AppendLine();

                            sb.AppendLine("InnerException:");
                            this.AppendException(sb, format, e.InnerException);
                        }
                        else
                        {
                            for( int i = 0; i < aggregateException.InnerExceptions.Count; ++i )
                            {
                                ...
                                sb.AppendLine("]:");
                                this.AppendException(sb, format, aggregateException.InnerExceptions[i]);
                            }
                        }
                    }
                }
                ...
                protected override string ApplyFormat( string format, object arg )
                {
                    ...
                    {
                        return '"' + ((Substring)arg).ToString().Replace("\"", "\"\"") + '"';
                    }
                    else if( arg is byte[] )
                    {
                    ...
                    {
                        var sb = new StringBuilder();
                        this.AppendException(sb, format, (Exception)arg);
                        return sb.ToString();
                    }
